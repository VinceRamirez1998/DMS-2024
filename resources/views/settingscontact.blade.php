  ...
  <div class="w-screen py-20">
    ...
    <div class="col-span-12 lg:col-span-6 lg:col-start-4 flex bg-[#bd8889] rounded-md p-3 border-2 border-red-500">
      <div class="flex flex-col justify-center w-full">
        {{-- Change Password --}}
        <form action="{{ route('settings.contact.update') }}" method="POST" class="mx-auto">
        @csrf
        <div class="grid grid-cols-5 gap-4 md:gap-4 mt-5 pb-5">
          {{-- Current Password --}}
          <div class="col-span-5 md:col-span-5">
            <div class="flex flex-col">
              <p>Current Contact No.</p>
              <input type="text" class="w-full py-1 rounded-md p-2 border-2 border-red-500 {{ auth()->user()->phone ? 'text-black' : 'text-gray-400' }}" value="{{ auth()->user()->phone ?? 'Contact number not set yet.' }}" readonly>
            </div>
          </div>
          {{-- New Password --}}
          <div class="col-span-5 md:col-span-5">
            <div class="flex flex-col">
                <p>New Contact No.</p>
                <input name="contact" type="text" class="w-full py-1 rounded-md p-2 border-2 border-red-500" placeholder="Enter New Contact No.">
            </div>
          </div>
